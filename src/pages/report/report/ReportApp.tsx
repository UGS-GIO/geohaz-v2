...
import CoverPage, { CoverPageProps } from './reportParts/CoverPage';
import SummaryPage, { SummaryPageProps } from './reportParts/SummaryPage';
...
function ReportApp(props: Aoi) {
  ...
  const [hazardReferences, setHazardReferences] = useState<HazardText[]>([]);
  ...
  const [reportTextMap, setReportTextMap] = useState<SummaryPageProps & CoverPageProps & { OtherGeologicHazardResources: string }>({
    groupToHazardMap: {},
    hazardToUnitMap: {},
    Top: "",
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const formattedHazardIntroText = hazardIntroText.map(({ Hazard, Text }) => ({ Hazard, Text }));
      const formattedHazardReferences = hazardReferences.map(({ Hazard, Text }) => ({ Hazard, Text }));
      ...
      const reportTextMapBuilder: SummaryPageProps & CoverPageProps & { OtherGeologicHazardResources: string } = {
        groupToHazardMap: {},
        hazardToUnitMap: {},
        Top: "",
        'Table1headingautogenerated table': "",
      ...
      reportTextRows.forEach(({ Section, Text }) => {
        if (Section in reportTextMapBuilder) {
          (reportTextMapBuilder as any)[Section] = Text;
        }
      });
      ...
      setHazardIntroText(formattedHazardIntroText);
      setHazardReferences(formattedHazardReferences);
                ...
                {hazardIntroText && hazardReferences && hazardToUnitMap && groupToHazardMap[groupName].map(hazardCode => {
                  ...
                  return (
                    <Hazard name={units[0].HazardName || ''} group={groupName} introText={introText || ''} key={hazardCode} code={hazardCode}>
                      ...
                      <References references={references.map(({ Text }: { Text: string }) => Text)}></References>
                    </Hazard>
                  )
                })}
              </Group>
