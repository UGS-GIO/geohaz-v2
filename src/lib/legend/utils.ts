...
const getWMSLayerRenderer = async (layer: __esri.WMSLayer) => {
    ...
    try {
        ...
        const previews = rules.map((rule) => {
            ...
            const isAutoGeneratedDefault = rule.symbolizers?.length === 1 && 
                pointSymbolizer &&
            ...
            if (isAutoGeneratedDefault) {
                ...
                // No content in the SVG - it will be invisible
                
                return {
                    type: 'regular-layer-renderer',
                    label: rule.title || rule.name, // Use the rule name/title as text
                    renderer: emptySvg,
